...
> 17 bytes header + ciphertext + auth tag
...
- **Compact** — 25–33 bytes of envelope overhead (depending on cipher suite)
...
2. Take the first 4 bytes of the digest (e.g., `0xab 0x77 0x88 0xd2`)
...
The authentication tag is produced automatically by the AEAD cipher during encryption. Its length depends on the cipher suite: **8 bytes** for CCM suites, **16 bytes** for GCM and ChaCha20-Poly1305. The tag covers both the plaintext (the headless inner frame) and the Associated Authenticated Data (AAD = envelope header, bytes 0–16). There is no separate hash computation.
...
**Transmission:** Only the 4-byte counter and 4-byte device hash are sent explicitly. The receiver reconstructs the full nonce using the Flags byte (byte 0 of the envelope), Device Hash, Counter, and the nonce length from the cipher suite table.
...
**Total overhead:** 25–33 bytes, depending on cipher suite. Formula: `17 (header) + tag_length` where tag_length is 8 bytes for CCM suites or 16 bytes for GCM / ChaCha20-Poly1305. No padding is required.
...
- **Max TagoTiP/S envelope size:** With the envelope overhead of `17 (header) + tag_length` bytes, the server MUST reject any envelope larger than **16,384 + 17 + tag_length** bytes (16,409 for CCM suites with 8-byte tag; 16,417 for GCM/ChaCha20 suites with 16-byte tag).
...
5. **Construct the nonce.** Length depends on cipher suite (see §3.3): 13 bytes for CCM, 12 bytes for GCM/ChaCha20. `[Flags (1B)] [zero-padding] [Device Hash (4B)] [Counter (4B)]`.
6. **Encrypt.** Use the AEAD cipher indicated by the Flags byte with the Encryption Key, nonce, plaintext = headless inner frame bytes, and AAD = the 17-byte header. The cipher produces the ciphertext (same length as plaintext) and the authentication tag.
7. **Assemble the envelope.** Concatenate: `[Header (17B)] + [Ciphertext (NB)] + [Auth Tag]`.
...
4. **Reconstruct the nonce.** Use the cipher suite to determine nonce length (see §3.3): `[Flags (1B)] [zero-padding] [Device Hash (4B)] [Counter (4B)]`.
5. **Decrypt.** Use the AEAD cipher from the Flags byte with the Encryption Key, nonce, ciphertext + auth tag, and AAD = bytes 0–16 (the 17-byte header). The cipher verifies integrity automatically — if the authentication tag does not match, decryption fails. Discard the envelope on authentication failure.
...
Step 2: Build header (AAD) = [0x80] + [0x00000001] + [Auth Hash (8B)] + [Device Hash (4B)] → 17 bytes
Step 3: Nonce (12 bytes for ChaCha20) = 0x80 000000 ab7788d2 00000001
                                        ↑flags ↑ 3 zeros ↑ dev hash ↑ counter
Step 4: ChaCha20-Poly1305 encrypt (key, nonce, plaintext=20B inner frame, AAD=17B header)
        → 20 bytes ciphertext + 16 bytes auth tag
Step 5: Envelope = [Header (17B)] + [Ciphertext (20B)] + [Auth Tag (16B)]
Total: 1 + 4 + 8 + 4 + 20 + 16 = 53 bytes
```
...
8. Reconstruct the nonce using the cipher suite's nonce length (see §3.3): `[Flags] [zero-padding] [Device Hash] [Counter]`
9. Decrypt with the cipher suite's AEAD algorithm using the Encryption Key, nonce, ciphertext + auth tag, and AAD = bytes 0–16 (the 17-byte header). The cipher verifies integrity automatically. If authentication fails and multiple candidate devices were identified in step 7, retry decryption with the next candidate's Encryption Key. If all candidates fail, discard the envelope and respond with `ACK|ERR|auth_failed`.
...
> **Note:** 25 bytes overhead with AES-128-CCM (8B tag), 33 bytes with GCM or ChaCha20-Poly1305 (16B tag).
